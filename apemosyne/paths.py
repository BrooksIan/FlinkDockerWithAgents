...
from apemosyne.constants import DEFAULT_PROFILE, FULL_PROFILE
...
def runtime_dir(root: Path | None = None) -> Path:
    """Generic Flink Agents runtime helpers (``apemosyne/runtime/``)."""
...
def configure_runtime_sys_path(
  ...
  By default honeypot modules are loaded only for the ``full`` compose profile.
    ...
    if include_honeypot is None:
        active = (profile or os.environ.get("APEMOSYNE_PROFILE", DEFAULT_PROFILE)).strip().lower()
