...
from apemosyne.constants import DEFAULT_PROFILE, FULL_PROFILE, normalize_profile
...
def default_flink_rest_port(profile: str | None = None) -> int:
    ...
    if env:
        return int(env)
    active = normalize_profile(profile or os.environ.get("APEMOSYNE_PROFILE", DEFAULT_PROFILE))
