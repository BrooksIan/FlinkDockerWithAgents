...
from apemosyne.pipelines.window_policies import GAP_POLICY_DEFAULT
...
_props = {{
    ...
    "group.id": "apemosyne-pipeline-{topic.replace('.', '-')}",
...
def cluster_window_block(window_config: WindowNodeConfig) -> tuple[list[str], str]:
    ...
    imports = [
        ...
        "from apemosyne.pipelines.window_ops import GenericSessionSummaryFunction",
    ]

    if window_config.gap_policy == GAP_POLICY_SESSION_DETECT:
        imports.append("from apemosyne.pipelines.window_ops import PolicyGapExtractor")
        gap_expr = f'PolicyGapExtractor("{GAP_POLICY_SESSION_DETECT}", gap_ms={window_config.gap_ms})'
    else:
        imports.append("from apemosyne.pipelines.window_ops import FixedGapExtractor")
...
def cluster_agent_logic_block(agent: str, window_config: WindowNodeConfig) -> tuple[list[str], str]:
    ...
    return (
        ["from apemosyne.pipelines.window_policies import prepare_agent_input"],
        (
...
""".strip("\n")
    imports.append("from apemosyne.kafka_sources import cluster_kafka_bootstrap_servers")
    return imports, block
