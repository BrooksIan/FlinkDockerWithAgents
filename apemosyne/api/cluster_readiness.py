...
def cluster_readiness(settings: ApiSettings) -> dict[str, Any]:
    ...
    checks.append(
        _check(
            ...
            if image_ok
            else f"Build image: apemosyne build",
            required=False,
        )
    )
    ...
    checks.append(
        _check(
            ...
            "running" if jm_running else "Start stack: apemosyne up",
        )
    )
    ...
    checks.append(
        _check(
            ...
            "running" if tm_running else "TaskManagers execute jobs — run: apemosyne up",
        )
    )
    ...
    if jm_running and jm_id:
        ...
        checks.append(
            _check(
                ...
                if jars_ok
                else "Submit once or run apemosyne test launch --cluster to bootstrap",
                required=False,
            )
        )
