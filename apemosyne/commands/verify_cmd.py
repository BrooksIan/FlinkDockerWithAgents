...
from apemosyne.constants import DEFAULT_PROFILE, VERIFY_TIERS, normalize_verify_profile
from apemosyne.docker_utils import IMAGE_NAME, IMAGE_TAG, project_root
from apemosyne.manifests import VerifyStep, get_verify_tier, load_verify_tiers
from apemosyne.paths import runtime_src_paths
...
def _docker_run_cmd(step: VerifyStep, root: Path) -> List[str]:
    ...
    if options.get("mount_project"):
        ...
        if manifest_name:
            from apemosyne.manifests import load_manifest
...
def _run_cli_step(step: VerifyStep, root: Path) -> int:
    ...
    if command == "validate":
        cli_args = [sys.executable, "-m", "apemosyne.main", "config", "validate"]
        if profile:
    ...
    base = [sys.executable, "-m", "apemosyne.main", command]
    ...
    *,
    ...
    if tier_name == "full":
        typer.echo(
            ...
            "  COWRIE_DURABLE_TOOLS=0 apemosyne verify --tier nightly --profile honeypot"
        )
    return 0
