...
from apemosyne.api.services import pipeline_health
from apemosyne.constants import DEFAULT_PROFILE
from apemosyne.docker_utils import compose_available, container_id, image_exists
...
def doctor_platform(
    ...
    typer.echo("Apemosyne platform doctor")
    typer.echo("")
    ...
    if image_ok:
        ...
        _line("agent_flink_image", "WARN", "run: apemosyne build")
    ...
    if flink_up:
        ...
        if docker_ok and not jm:
            _line("flink reachable", "WARN", flink.get("error", "start: apemosyne up"))
        else:
            critical_fail = True
    ...
    if settings.api_key:
        _line("api auth", "OK", "APEMOSYNE_API_KEY configured")
    else:
        _line("api auth", "WARN", "APEMOSYNE_API_KEY not set — protected routes are open")
    ...
    try:
        ...
        _line("api process", "WARN", "start with: apemosyne api start")
