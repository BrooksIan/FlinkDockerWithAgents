...
def ensure_kafka_topics(profile: str = "full") -> None:
    ...
    if rc != 0:
        typer.echo(
            "Failed to ensure Kafka topics. Check: apemosyne logs flink-pipeline-supervisor",
            err=True,
        )
        raise typer.Exit(rc)
...
def ensure_flink_jobs(profile: str = "full") -> None:
    ...
    if rc != 0:
        typer.echo(
            "Failed to ensure Flink jobs. Check: apemosyne logs flink-pipeline-supervisor",
            err=True,
        )
        raise typer.Exit(rc)
...
def up(
    ...
    if build_image and not image_exists():
        ...
        from apemosyne.commands.build import build_image as do_build
    ...
    if profile == "full" and ensure_kafka:
        try:
            ...
            typer.echo(
                ...
                "Supervisor will retry; or run: apemosyne ensure-kafka-topics",
                err=True,
            )
    ...
    if profile == "full" and ensure_flink:
        try:
            ...
            typer.echo(
                ...
                "Sidecars will retry; or run: apemosyne ensure-flink-jobs",
                err=True,
            )
