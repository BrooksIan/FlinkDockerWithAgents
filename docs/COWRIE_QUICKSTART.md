...
Start the stack (includes Kafka/ZooKeeper in `docker-compose-cowrie.yml`).
...
```bash
docker compose -f docker-compose-cowrie.yml up -d --scale log-processor=0
```
...
```bash
flink-cowrie test phase1          # submit normalize job, wait for RUNNING
flink-cowrie test phase1 --e2e    # publish test event, verify cowrie.normalized schema
flink-cowrie test phase2          # submit workflow job, wait for RUNNING
flink-cowrie test phase2 --e2e    # publish normalized test event, verify cowrie.alerts
flink-cowrie test phase3          # smoke: ReAct augmentor imports + Cloudera config
flink-cowrie test phase3 --e2e    # Cloudera ReAct on sample event + cowrie.react_alerts
flink-cowrie test production          # smoke: topic routing + hot-path policy
flink-cowrie test production --e2e  # one event → cowrie.alerts + cowrie.react_alerts (if Cloudera)
...
```bash
docker compose -f docker-compose-cowrie.yml logs -f flink-pipeline-supervisor cowrie-kafka-shipper kafka-react-augmentor kafka-alerts-to-dashboard
```
...
### Change Ports
Edit `docker-compose-cowrie.yml`:
```yaml
cowrie:
  ports:
...
```bash
# Check logs
docker compose -f docker-compose-cowrie.yml logs cowrie
