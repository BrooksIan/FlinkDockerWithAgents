...
from flink_cowrie.docker_utils import (
    compose_cmd,
    compose_file,
    container_id,
    docker_exec,
...
PROFILE_HELP = "Stack profile: 'full' (Cowrie + Kafka + dashboard) or 'minimal' (Flink only)"
...
def ensure_kafka_topics(profile: str = "full") -> None:
    """Create all Cowrie pipeline Kafka topics if missing (full stack only)."""
    if profile != "full":
    ...
    typer.echo("Ensuring Cowrie pipeline Kafka topics exist...")
    rc = docker_exec(cid, _ENSURE_KAFKA_TOPICS_CMD, interactive=False)
    if rc != 0:
        typer.echo(
            "Failed to ensure Kafka topics. Check: flink-cowrie logs flink-pipeline-supervisor",
            err=True,
        )
        raise typer.Exit(rc)
...
def ensure_flink_jobs(profile: str = "full") -> None:
    ...
    typer.echo("Ensuring Cowrie Phase 1 / 1.5 / 2 Flink jobs are RUNNING...")
    rc = docker_exec(cid, _ENSURE_FLINK_JOBS_CMD, interactive=False)
    if rc != 0:
        typer.echo(
            "Failed to ensure Flink jobs. Check: flink-cowrie logs flink-pipeline-supervisor",
            err=True,
        )
        raise typer.Exit(rc)
...
def ensure_kafka_topics_cmd(
    ...
    """Ensure all Cowrie pipeline Kafka topics exist."""
...
def up(
    ...
    if profile == "full":
        (root / "cowrie-logs").mkdir(exist_ok=True)
        (root / "cowrie-data").mkdir(exist_ok=True)

    if build_image and not image_exists():
        typer.echo("Flink image not found — building first...")
        from flink_cowrie.commands.build import build_image as do_build
    ...
    if profile == "full" and ensure_kafka:
        try:
            ...
            typer.echo(
                ...
                "Supervisor will retry; or run: flink-cowrie ensure-kafka-topics",
                err=True,
            )
    ...
    if profile == "full" and ensure_flink:
        try:
            ...
            typer.echo(
                ...
                "Sidecars will retry; or run: flink-cowrie ensure-flink-jobs",
                err=True,
            )
