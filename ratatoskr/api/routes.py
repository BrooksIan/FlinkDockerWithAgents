...
from ratatoskr.agents.registry import AgentRegistryError
from ratatoskr.api import flink_client, services
from ratatoskr.api.auth import require_api_key
from ratatoskr.api.config import ApiSettings
from ratatoskr.api.events import event_stream
from ratatoskr.api.observability import refresh_flink_gauges
...
class McpCatalogServerCreate(BaseModel):
    ...
    required_secrets: list[dict[str, str]] | None = None
...
@router.post("/mcp/catalog/servers", tags=["mcp"], dependencies=[Depends(require_api_key)])
def mcp_catalog_add_server(body: McpCatalogServerCreate) -> dict[str, Any]:
    try:
        return services.add_mcp_catalog_server_api(body.model_dump())
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@router.post(
    "/agent-definitions/{definition_id}/run-local",
    tags=["designer"],
    dependencies=[Depends(require_api_key)],
)
