...
from ratatoskr.api.config import ApiSettings, load_settings
from ratatoskr.api.observability import configure_logging, metrics_payload
from ratatoskr.api.routes import router
...
def create_app(settings: ApiSettings | None = None) -> FastAPI:
    ...
    os.environ.setdefault("RATATOSKR_PROFILE", cfg.default_profile)
    configure_logging(json_logs=cfg.log_json)

    app = FastAPI(
        title="Ratatoskr Control API",
    ...
    async def metrics_middleware(request: Request, call_next):
        ...
        from ratatoskr.api.observability import track_request
