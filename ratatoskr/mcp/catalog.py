...
from ratatoskr.paths import examples_dir, project_root
...
def load_builtin_mcp_catalog(*, root: Optional[Path] = None) -> McpCatalog:
...
def load_mcp_catalog(*, root: Optional[Path] = None) -> McpCatalog:
    """Builtin YAML catalog merged with user-added servers from designer.db."""
    from ratatoskr.mcp.custom_catalog import custom_servers_as_entries

    builtin = load_builtin_mcp_catalog(root=root)
    custom_servers = custom_servers_as_entries(root=root)
    if not custom_servers:
        return builtin

    categories: list[McpCatalogCategory] = list(builtin.categories)
    categories.append(
        McpCatalogCategory(
            id="custom",
            label="Custom",
            description="User-added MCP servers from Settings.",
            servers=custom_servers,
        )
    )
    return McpCatalog(categories=tuple(categories))
...
def mcp_catalog_response(*, root: Optional[Path] = None) -> dict[str, Any]:
    ...
    for category in catalog.categories:
        ...
        for server in category.servers:
            source = "custom" if category.id == "custom" else "builtin"
            servers.append(
                {
                    "id": server.id,
