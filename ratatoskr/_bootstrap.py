...
def _load_pyc(qualified: str) -> None:
    if qualified in sys.modules:
        return
    relative = qualified.removeprefix("ratatoskr.")
    if _source_py_path(relative) is not None:
        import importlib

        importlib.import_module(qualified)
        legacy = qualified.replace("ratatoskr", "flink_cowrie", 1)
        if qualified in sys.modules:
...
def install_aliases() -> None:
    ...
    class _LegacyAliasFinder:
        def find_spec(self, fullname, path=None, target=None):
            if fullname == "flink_cowrie" or fullname.startswith("flink_cowrie."):
                alias = fullname.replace("flink_cowrie", "ratatoskr", 1)
    ...
    for mod in (
        "ratatoskr.constants",
        "ratatoskr.docker_utils",
        "ratatoskr.paths",
        "ratatoskr.manifests",
        "ratatoskr.startup_modes",
        "ratatoskr.commands",
    ):
        importlib.import_module(mod)
    sys.modules["flink_cowrie.commands"] = sys.modules["ratatoskr.commands"]

    for name in _PYC_MODULES:
        _load_pyc(f"ratatoskr.{name}")

    if "ratatoskr.checks" in sys.modules:
        sys.modules["flink_cowrie.checks"] = sys.modules["ratatoskr.checks"]
    sys.modules["flink_cowrie.commands"] = sys.modules["ratatoskr.commands"]

    from ratatoskr.commands import test_cmd
...
def get_app():
    """Return the Typer application (prefers ``ratatoskr.cli`` source)."""
    install_aliases()
    from ratatoskr.cli import app as source_app
