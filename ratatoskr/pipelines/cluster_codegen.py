...
from ratatoskr.agents.published_copy import published_cluster_import_line
from ratatoskr.agents.registry import get_agent_spec
from ratatoskr.pipelines.executor import linear_execution_order
...
from ratatoskr.pipelines.window_codegen import (
...
def cluster_job_name(pipeline: Pipeline) -> str:
    return f"Ratatoskr Pipeline: {pipeline.name} [{pipeline.id}]"


def cluster_runner_relpath(pipeline_id: str) -> str:
    return f".ratatoskr/pipelines/{pipeline_id}/run_cluster.py"


def cluster_bridge_window_relpath(pipeline_id: str) -> str:
    return f".ratatoskr/pipelines/{pipeline_id}/run_cluster_window.py"


def cluster_bridge_agent_relpath(pipeline_id: str) -> str:
    return f".ratatoskr/pipelines/{pipeline_id}/run_cluster_agent.py"


def compiled_pipeline_dir(pipeline_id: str, *, root: Path | None = None) -> Path:
    repo = root or project_root()
    directory = repo / ".ratatoskr" / "pipelines" / pipeline_id
            ...
            if prev_kind == "agent":
                ...
                if edge and edge.mapping:
                    ...
                    map_functions.append(
                        f"def {fn_name}(row):\n"
                        f"    from ratatoskr.pipelines.executor import apply_edge_mapping\n"
...
def _generate_cluster_window_runner(
    ...
    *,
    ...
    if uses_agents_env:
        lines.append("    from ratatoskr.runtime.flink_agents_bootstrap import patch_flink_agents_version")
    ...
    for imp in sorted(extra_imports):
        lines.append(f"    {imp}")
    if uses_kafka_jars:
        lines.append("    from ratatoskr.runtime.kafka_jars import attach_kafka_jars")
...
def generate_cluster_bridge_agent_runner(pipeline: Pipeline, *, root: Path | None = None) -> str:
    ...
    lines.append("    from flink_agents.api.execution_environment import AgentsExecutionEnvironment")
    lines.append("    from ratatoskr.runtime.kafka_jars import attach_kafka_jars")
    lines.append("    from ratatoskr.kafka_sources import cluster_kafka_bootstrap_servers")
    ...
    lines.extend(
        [
            ...
            f'        "group.id": "ratatoskr-bridge-{pipeline.id}",',
