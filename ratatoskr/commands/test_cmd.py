...
from ratatoskr.constants import DEFAULT_PROFILE, FULL_PROFILE, PROFILE_HELP
...
from ratatoskr.paths import honeypot_available, honeypot_module_rel, project_root
...
def _generic_validate_paths(root: Path) -> list[str]:
    return [
        ...
        "ratatoskr/manifests/test-launch.yaml",
...
def _run_launch_test_in_container(
    ...
    from ratatoskr.copy_manifest import copy_manifest_to_cluster
    ...
    if not cid:
        typer.echo(
            ...
            f"  ratatoskr up --profile {profile}",
            err=True,
        )
        raise typer.Exit(1)
...
def _load_legacy_test_cmd():
    ...
    qualified = "ratatoskr.commands._test_cmd_legacy"
