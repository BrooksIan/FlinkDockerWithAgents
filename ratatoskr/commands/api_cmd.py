...
app = typer.Typer(help="Ratatoskr control API (for dashboards).")


@app.command("start")
def api_start(
    host: str = typer.Option("", "--host", help="Bind host (default: RATATOSKR_API_HOST)"),
    port: int = typer.Option(0, "--port", help="Bind port (default: RATATOSKR_API_PORT)"),
    ...
    typer.echo(f"Starting Ratatoskr API on http://{bind_host}:{bind_port}")
