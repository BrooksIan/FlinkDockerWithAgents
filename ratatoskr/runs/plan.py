...
CLUSTER_JOB_NAMES: dict[str, str] = {
    "workflow_counter": "Ratatoskr Workflow Counter",
    "react_echo": "Ratatoskr React Echo",
    "react_double_value": "Ratatoskr React Double Value",
}
...
def cluster_job_name_for_pipeline(pipeline: "Pipeline") -> str:
    from ratatoskr.pipelines.cluster_codegen import cluster_job_name
...
def pipeline_execution_plan_for_run(agent: str) -> list[dict[str, Any]]:
    ...
    if pipeline is None:
        return []
    from ratatoskr.pipelines.cluster_codegen import pipeline_execution_plan
